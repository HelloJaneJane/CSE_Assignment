...
public class testGenetic {
	...
	// population size
	static int pp_siz = 5000;
	// parent size (population size * 0.1)
	static int pr_siz = 500;
	// crossover로 만들어진 다음 세대 size
	static int cross_siz = 4000;
	// mutation으로 만들어진 다음 세대 size
	static int mutat_siz = 500;
	
	// 토너먼트 셀렉션에서 k번 선택
	static int k_siz = 7;
	
	// 최대 num (십만번 안에 답 안 나오면 안 되는 RNA라 생각하고 걍 kill)
	static int max_num = 100000;
	
	
	static int failnum=0;
	static int firstparnum=0;
	static int crossnum=0;
	static int mutatnum=0;
	static int gensum=0;
	...
	public static void main(String args[]) throws IOException {
		...
		String path = args[1] + "/testresult" + args[0] + ".txt"; // 파일 저장 경로
		...
		result += "\n\n1000 testcases...\n";
		result += Integer.toString(failnum) +"times fail\n";
		result += "average of generation number : " + Double.toString((double)gensum/100) +"\n\n";
		
		result += "firstparnum : " + Integer.toString(firstparnum) +"\n";
		result += "crossnum : " + Integer.toString(crossnum) +"\n";
		result += "mutatnum : " + Integer.toString(mutatnum) +"\n";
